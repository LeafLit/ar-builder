...
## 2026-06-25 补充：二维码标记识别实现说明

- 测试页的自动识别输入方式现在包含三类：`相机分类`、`颜色标记`、`二维码标记`。
- `二维码标记` 的核心实现位于 `src/features/testing/qrMarkerRecognizer.ts`，它会复用现有摄像头服务，把视频帧缩小后交给 `jsqr` 解码。
- 二维码内容 `ARBUILDER:1`、`ARBUILDER:2`、`ARBUILDER:3` 会映射到当前项目的第 1、2、3 个状态。映射不存在时不输出识别结果，避免误触发。
- `createQrMarkerRecognizer` 对外仍暴露成项目已有的 `StateRecognizer` 形态，所以测试页后面的 AR 输出、状态计数、震动、音频等逻辑不需要单独分叉。
- 如果以后继续增加图片标记、条形码或其他稳定输入方式，建议继续做成独立 recognizer，再在 `TestScreen.tsx` 的输入模式分支里接入。
